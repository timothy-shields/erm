...
namespace ermc
{
    ...
    class App
    {
        ...
        [Verb(Description = "Compile a .erm file to a .dot file and then render that to a file of the specified format using Graphviz.")]
        public static void Render(
            [Required] string inputErmPath,
            GraphvizOutputFormat format = GraphvizOutputFormat.svg)
        {
            var dotPath = Path.ChangeExtension(inputErmPath, "dot");
            var outputPath = Path.ChangeExtension(inputErmPath, format.ToString().ToLower());

            Compile(inputErmPath);

            var graphvizBin = ConfigurationManager.AppSettings["graphvizBin"];
            var service = new GraphvizService(graphvizBin);
            service.DotAsync(dotPath, outputPath, format, CancellationToken.None).Wait();
        }
